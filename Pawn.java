...
public class Pawn extends Piece{
    ...
    public ArrayList<Square> getMoves(GameBoard currBoard, Square square) {
        ...
        if (isValid(forward1) && currBoard.getPiece(forward1).getColor() == Color.NONE) {
            ...
            if (isValid(forward1) && currBoard.getPiece(forward2).getColor() == Color.NONE && onStart) {
        ...
        if (isValid(forward1) && currBoard.getPiece(left).getColor() == other) {
            validMoves.add(left);
        }
        if (isValid(forward1) && currBoard.getPiece(right).getColor() == other) {
